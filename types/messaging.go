...
func (s StatusMessage) View() map[string]uint {
	mapview := make(map[string]uint)
...
// RumorBySeq sorts rumor by origin
type RumorBySeq []Rumor

func (r RumorBySeq) Len() int {
	return len(r)
}

func (r RumorBySeq) Swap(i, j int) {
	r[i], r[j] = r[j], r[i]
}

func (r RumorBySeq) Less(i, j int) bool {
