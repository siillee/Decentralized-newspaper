...
func (c Catalog) Update(key, peer string) {
	_, ok := c[key]
	if !ok {
		c[key] = make(map[string]struct{})
	}
	var emptyStruct struct{}
	c[key][peer] = emptyStruct
}

func (c Catalog) GetRandomPeer(key string) (string, error) {
	bag, ok := c[key]
	if !ok {
		return "", xerrors.New("no peer for that element")
	}
	peers := make([]string, len(bag))
	i := 0
	for k := range bag {
		peers[i] = k
		i++
